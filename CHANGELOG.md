...
- **Complete usage guide** (`docs/USAGE.md`) — a single document covering every widget and its key controls, the Site Info hub concept, the full `window.Paradise` JavaScript API, ready-made recipes (e.g. opening the Off-Canvas Menu from the Bottom Nav center button via `ebn:hook:*` events), and a troubleshooting section for the most common configuration mistakes. Linked from `README.md` and the WP.org readme FAQ.
- **In-editor control tooltips on the controls that most affect behaviour** — every *Source* / *Mode* / *Text Mode* selector (Google Map, FAQ, Social Links, Author Card, Bottom Nav, Phone Button) and the phone *Country Code* / *Display Format* controls now carry a precise `description` explaining what each option does and the common pitfall (e.g. "switch Text Mode to Custom Text to reveal the Button Text field"; "enter the number without the country code — it is added automatically and never doubled"). Also the Bottom Nav badge JS API, the Business Hours "Open now" timezone behaviour, the Off-Canvas `Paradise.toggleOffCanvas` hook, and the Announcement Bar per-browser dismissal storage. Styling controls are intentionally left untouched to keep tooltips meaningful (≈53 of 423 controls documented).
...
- **No more inline `<style>` / `<script>` — everything enqueued (WP.org guideline).** The editor empty-widget placeholder CSS moved from an inline `<style>` in `Paradise_Widget_Base` to `assets/css/editor-placeholder.css`, enqueued into the Elementor preview via `elementor/preview/enqueue_styles`. The Import/Export page's inline `<script>` (enable the Import button once a file is chosen) moved into the already-enqueued `assets/js/admin.js`, guarded so it no-ops on other admin pages.
- **Removed `load_plugin_textdomain()`** — WordPress.org auto-loads translations for hosted plugins (since WP 4.6) and this plugin requires WP 6.1+, so the call (and its `plugins_loaded` hook) is unnecessary.
- **Documented the Google Maps embed as an external service** — added an `== External services ==` section to `readme.txt` explaining that the Google Map widget loads an `<iframe>` from Google (what it is, what data the visitor's browser sends and when, with links to Google's Terms and Privacy Policy).

### Fixed

- **readme.txt FAQ — corrected the "trigger Off-Canvas from Bottom Navigation" answer.** It previously described a non-existent "JS trigger" control on the Off-Canvas widget. The real mechanism: the Bottom Nav center button's *JS Hook* action dispatches an `ebn:hook:<name>` event on `document`; you listen for it and call `Paradise.toggleOffCanvas(id)`. The FAQ now shows the working snippet.
...
- **FAQ Accordion — same JSON-LD `</script>` breakout fixed.** The `FAQPage` schema block still used `JSON_UNESCAPED_SLASHES`, so a question or answer containing `</script>` could break out of the inline block. Dropped the flag (matches LocalBusiness Schema). Also removed `JSON_UNESCAPED_SLASHES` from the Import/Export download for consistency — no occurrences remain anywhere.
...
- **Business Hours — PHP "Undefined array key" notice** in `format_time()` when a stored time had no colon; now guarded.
- **Phone Link / Button / Floating Call — country code added twice.** When a user typed the country code without a leading `+` (exactly what the placeholders invite, e.g. `1 888 …`), `build_phone_href()` prepended the code again, producing un-dialable `tel:`/`wa.me` numbers like `+11888…`. It no longer double-prefixes when the number already starts with the country code, and returns an empty href (text, no link) when there are no dialable digits.
- **Author Card — social link rendered with `esc_attr()` instead of `esc_url()`**, which would not strip a `javascript:` scheme from a user profile field. Now uses `esc_url()`.
- **Bottom Navigation Bar — every placeholder item lit up as "active" on the home page.** Items with no URL fell back to the site home URL and matched it. Active state now ignores URL-less items and compares normalized paths (ignoring host, query string, and trailing slash) consistently on server and client.
- **Bottom Navigation Bar — Speed Dial center button now toggles closed on a second tap** (was hard-coded to only open) and the widget no longer double-initializes from both `DOMContentLoaded` and the Elementor `element_ready` hook (which leaked duplicate overlays and `document` key listeners, especially in the editor).
- **Bottom Navigation Bar — guarded Speed Dial / item icon and label array keys** against "Undefined array key" notices on incomplete saved/imported data; inactive items no longer emit `aria-current="false"` (the attribute is removed instead).
- **Announcement Bar & Cookie Consent Bar — buttons silently dead in Safari private mode / when storage is blocked.** An unguarded `localStorage`/`sessionStorage` access threw and aborted the whole init loop, so Accept/Decline/Close never got wired and bars reappeared every visit. All storage access is now wrapped in try/catch.
- **Translation loading no longer triggers WP 6.7+ `_doing_it_wrong` notices** for the `paradise-widgets-for-elementor` textdomain. Two changes: (1) the FAQ CPT loader moved from `plugins_loaded` to `init` priority 5 so it runs after the textdomain is ready, and (2) `Paradise_EW_Admin::feature_enabled()` now reads the stored option directly instead of walking `widget_registry()` (whose label/description strings call `__()`). Previously, calling `feature_enabled( 'faq_cpt' )` during `plugins_loaded` triggered a translation lookup for every widget label and produced ~230 notices per page load with full stack traces — bloating `debug.log` to multiple megabytes and noticeably slowing the Elementor editor in dev environments.
- **Saving one admin page no longer wipes the other's toggles.** The "Elementor Widgets" page (per-widget enable/disable) and the "Settings" page (feature flags) share a single WordPress option (`paradise_ew_settings`) and a single `register_setting()` sanitize callback. The previous callback always re-evaluated *both* sections from `$_POST`, so saving the Settings page (whose form posts only feature fields) silently reset every widget to disabled — and vice versa. `sanitize_settings()` now reads a hidden `_section` marker each form posts (`widgets` or `features`) and only rewrites that section, preserving the other from the existing stored value. Import / Export passes `_section => 'all'` so a JSON import still rewrites both sections in one shot.
