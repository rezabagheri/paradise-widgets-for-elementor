...
- **SoundCloud widget** — embeds a SoundCloud single track or Set (playlist) via the official player iframe. Controls: URL (with Elementor Dynamic Tag support so ACF Pro users can bind it to a custom field on the current post), player style (*Visual* with artwork or *Classic* mini), accent colour (defaults to SoundCloud orange), auto-play, show comments, show related tracks, show uploader, plus max-width and border-radius style controls. Strict URL allowlist (`soundcloud.com`, `m.soundcloud.com`, `www.soundcloud.com`) deliberately rejects short links (`on.soundcloud.com`, the iframe can't follow redirects) and already-embedded URLs (`w.soundcloud.com`, would double-embed) so broken iframes are caught early with an editor placeholder. iframe uses `loading="lazy"` so off-screen embeds don't pull SoundCloud's ~500 KB of player JS until they scroll into view. This is **Phase 1** (single track + playlist URL). Phase 2 will add a *Custom Playlist* mode for ACF Repeater data using the SoundCloud Widget API.
