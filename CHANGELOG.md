...
- **SoundCloud widget** — embeds SoundCloud audio via the official player iframe with four content sources selected by a single `Source` SELECT: **Single Track or Set URL** (one URL, SoundCloud's player auto-detects track vs `/sets/` playlist), **ACF Repeater Playlist** (reads a Repeater field on the current post — `tracks` with `url` / `title` / `artist` / `description` sub-fields by default, all configurable; uses `get_field()` when ACF is active and falls back to `get_post_meta()` otherwise), and **Manual List Playlist** (an in-widget Elementor Repeater with the same four sub-fields per row, no external dep). The shared `URL` control for single mode supports Elementor Dynamic Tags so ACF Pro users can bind it without the widget needing to know about ACF. Style controls: player style (*Visual* with artwork or *Classic* mini), accent colour (defaults to SoundCloud orange `#ff5500`), auto-play, show comments, show related tracks, show uploader, max-width, border-radius. Strict URL allowlist (`soundcloud.com`, `m.soundcloud.com`, `www.soundcloud.com`) rejects short links (`on.soundcloud.com`, the iframe can't follow redirects) and already-embedded URLs (`w.soundcloud.com`, would double-embed). Both playlist modes share one iframe + a track list of accessible `<button>`s (not `<a>`, since clicking isn't navigation); clicks swap the audio in-place via `SC.Widget(iframe).load(url, opts)` so a 50-track playlist still loads SoundCloud's ~500 KB player JS only once. The Widget API itself (~10 KB at `w.soundcloud.com/player/api.js`) is lazy-loaded on the FIRST click in each playlist — pages where nobody touches the list pay nothing for it. Auto-advance on the SC `FINISH` event chains tracks for continuous play, stopping at the end. The iframe carries `loading="lazy"` so off-screen embeds don't pull the player JS until they scroll into view. Currently-playing track gets an accent-coloured left border and `aria-current="true"` so the keyboard / screen-reader experience matches the visual one.
